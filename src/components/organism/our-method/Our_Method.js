import React from 'react'

const Our_Method = () => {
    return (
        <div className="our-method-2">
            <div className="head-methods-2-1">
                <h2>Vår Metod</h2>
            </div>
            <div className="methods-2-1">
                <div className="move-2-1">
                    <h4>1. Flytten Går</h4>
                    <p>En partner ska flytta eller renovera kontoret och har dugliga möbler över som tänkt åka till tippen</p>
                </div>
                <div className="move-2-1">
                    <h4>2. Återliva Möbler</h4>
                    <p>Skippa tippen! 300k ton möbler slängs varje år. Kontakta oss på Gagnat eller logga in på platformen
                    </p>
                </div>
                <div className="move-2-1">
                    <h4>3. Inventering</h4>
                    <p>Ta bilder på möbler som ni vill bli av med, och lägg upp på platformen för att återlivas</p>
                </div>
                <div className="move-2-1">
                    <h4>4.Den perfekta matchen</h4>
                    <p>Medlemmar tingar möbler via platformen och blir matchad med den partner som har möbler över </p>
                </div>
                <div className="move-2-1">
                    <h4>5. Nya livet</h4>
                    <p>Partner och medlemmen kommer överrens om leverans. En rapport av miljöbesparingarna skickas till partnern samt miljöcertifikat
                    </p>
                </div>
            </div>
        </div>
    )
}

export default Our_Method
